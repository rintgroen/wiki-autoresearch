...
If this package was generated by Codex, the local git repository and first commit may already exist. If `git status` says the working tree is clean, skip to the next step.

If you are starting from an uncommitted folder, run:
...
5. Let VS Code create the remote and push the committed files.
